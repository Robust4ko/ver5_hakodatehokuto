...
function updateCurrentOverlay(pos) {
  ...
  // 現在位置マーカー（青い丸）— ラベルは一切使わない（[object Object]対策）
  if (!currentMarker) {
    ...
    if (currentMarker.setLabel) currentMarker.setLabel(null); // ← 念のため毎回オフ
  }
...
function startTracking() {
  ...
  watchId = navigator.geolocation.watchPosition(
    (pos) => {
      ...
      // 初回含め、ON中のみ現在地マーカーを表示（updateで生成）
    ...
    (err) => {
      stopTracking();
      if (err.code === err.PERMISSION_DENIED)      displayMessage(T('locPermissionDenied'));
      else if (err.code === err.TIMEOUT)           displayMessage(T('locTimeout'));
      else                                         displayMessage(T('locUnavailable'));
    },
